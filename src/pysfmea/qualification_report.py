...
<div class="card metric"><strong>{summary['semantic_missing_cases']}</strong><span>missing semantic cases</span></div>
<div class="card metric"><strong>{summary['semantic_mismatched_claims']}</strong><span>mismatched semantic claims</span></div>
...
const semanticDiagnostics=[];let retainedSemanticExamples=0,omittedSemanticExamples=0;data.repositories.forEach(r=>{{const d=r.semantic_diagnostics||{{examples:[],examples_omitted:0}};retainedSemanticExamples+=d.examples.length;omittedSemanticExamples+=d.examples_omitted||0;d.examples.forEach(example=>semanticDiagnostics.push({{repository:r.id,...example}}))}});$("semanticDiagnosticNotice").textContent=retainedSemanticExamples?`${{retainedSemanticExamples}} retained drift example(s) shown${{omittedSemanticExamples?`; ${{omittedSemanticExamples}} additional example(s) omitted by the bounded diagnostic limit.`:"."}}`:"No retained semantic drift examples. Exact aggregate metrics and evaluation artifacts remain the source of record.";let semanticDiagnosticPage=1,semanticDiagnosticPageSize=100,semanticDiagnosticPrintState=null;function renderSemanticDiagnostics(){{const query=$("semanticDiagnosticSearch").value.trim().toLowerCase(),all=semanticDiagnostics.filter(example=>JSON.stringify(example).toLowerCase().includes(query)),pages=Math.max(1,Math.ceil(all.length/semanticDiagnosticPageSize));semanticDiagnosticPage=Math.min(semanticDiagnosticPage,pages);const shown=all.slice((semanticDiagnosticPage-1)*semanticDiagnosticPageSize,semanticDiagnosticPage*semanticDiagnosticPageSize),rows=$("semanticDiagnosticRows");rows.replaceChildren();if(!shown.length){{const tr=document.createElement("tr"),td=node("td",semanticDiagnostics.length?"No semantic drift examples match this search.":"No retained semantic drift examples.");td.colSpan=8;tr.append(td);rows.append(tr)}}shown.forEach(example=>{{const tr=document.createElement("tr");[example.repository,label(example.kind),example.source,example.component,example.rule_id,example.field,Array.isArray(example.expected)?example.expected.join(", "):example.expected,Array.isArray(example.actual)?example.actual.join(", "):example.actual].forEach(x=>cell(tr,x));rows.append(tr)}});$("semanticDiagnosticCount").textContent=`${{all.length.toLocaleString()}} matching example(s)`;$("semanticDiagnosticPage").textContent=`Page ${{semanticDiagnosticPage}} of ${{pages}}`;$("semanticDiagnosticPrev").disabled=semanticDiagnosticPage===1;$("semanticDiagnosticNext").disabled=semanticDiagnosticPage===pages}}$("semanticDiagnosticSearch").addEventListener("input",()=>{{semanticDiagnosticPage=1;renderSemanticDiagnostics()}});$("semanticDiagnosticPrev").addEventListener("click",()=>{{semanticDiagnosticPage--;renderSemanticDiagnostics()}});$("semanticDiagnosticNext").addEventListener("click",()=>{{semanticDiagnosticPage++;renderSemanticDiagnostics()}});renderSemanticDiagnostics();window.addEventListener("beforeprint",()=>{{semanticDiagnosticPrintState={{query:$("semanticDiagnosticSearch").value,page:semanticDiagnosticPage,size:semanticDiagnosticPageSize}};$("semanticDiagnosticSearch").value="";semanticDiagnosticPage=1;semanticDiagnosticPageSize=20_000;renderSemanticDiagnostics()}});window.addEventListener("afterprint",()=>{{if(!semanticDiagnosticPrintState)return;$("semanticDiagnosticSearch").value=semanticDiagnosticPrintState.query;semanticDiagnosticPage=semanticDiagnosticPrintState.page;semanticDiagnosticPageSize=semanticDiagnosticPrintState.size;semanticDiagnosticPrintState=null;renderSemanticDiagnostics()}});
