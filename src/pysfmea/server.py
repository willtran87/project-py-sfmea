...
function renderAssurancePlan(){
  ...
  $('assuranceList').innerHTML=values.map((value,index)=>{
    ...
    return `<article class="assurance-card"><div class="eyebrow">${esc(value.id)} · ${esc(value.component)}</div><h3>${esc(value.title)}</h3><div class="assurance-meta"><span class="tag ${esc(value.priority)}">${esc(value.priority||'priority n/a')}</span><span class="tag">${esc(status)}</span><span class="tag">evidence: ${esc(evidence)}</span><span class="tag">implementation: ${esc(implementation)}</span><span class="tag">${esc(value.verification_method||'method pending')}</span></div><p><b>Stimulus:</b> ${esc(value.stimulus?.description||'Not defined')}</p>${gaps?`<details open><summary>Planning gaps (${value.planning_gaps.length})</summary><ul>${gaps}</ul></details>`:''}<details><summary>Acceptance criteria (${value.acceptance_criteria?.length||0})</summary><ul>${criteria}</ul></details>${form}</article>`;
