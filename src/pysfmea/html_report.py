...
<main class="main" id="mainContent" tabindex="-1">
  ...
  <div class="content">
    ...
    <section class="view" data-view="cross-reference" hidden><div class="section-head"><div><h2>Cross-reference evidence fabric</h2><p>One deterministic chain across scanner relationships, runtime observations, semantic analyzers, guidance, SFTA, findings, verification obligations, executions, and evidence.</p></div><button class="btn no-print" data-open-diagram="cross-reference-evidence-fabric">Open evidence diagram</button></div><div class="notice"><strong>Interpretation boundary:</strong> Populated links, model intersections, and multi-source agreement improve review leverage; they do not establish reachability, causality, completeness, compliance, verification success, or risk acceptance.</div><div class="metrics" id="crossReferenceMetrics"></div><div class="grid"><div class="card"><div class="card-head"><div><h3>Relationship consensus</h3><p>Independent channels fused by directed component pair.</p></div></div><div class="bars" id="crossReferenceConsensus"></div></div><div class="card"><div class="card-head"><div><h3>Semantic exposure coverage</h3><p>Components linked to exact bounded analyzer records.</p></div></div><div class="bars" id="crossReferenceSemanticCoverage"></div></div><div class="card"><div class="card-head"><div><h3>Compound model intersections</h3><p>Failure-mode chains where independently derived semantic models co-occur.</p></div></div><div class="bars" id="crossReferenceCompoundExposure"></div></div><div class="card"><div class="card-head"><div><h3>Prioritized discrepancies and intersections</h3><p>Scanner disagreement, missing links, and compound semantic exposures that deserve review.</p></div></div><div class="citation-list" id="crossReferenceLeads"></div></div></div><div class="card" style="margin-top:16px"><div class="card-head"><div><h3>Finding-to-evidence chains</h3><p>Lowest linkage-completeness chains are shown first. Completeness is relationship accounting only.</p></div></div><div class="citation-list" id="crossReferenceChains"></div></div><div class="card" style="margin-top:16px"><div class="card-head"><h3>Authority and limits</h3></div><ul id="crossReferenceLimits"></ul><p class="small mono" id="crossReferenceBinding"></p></div></section>
...
function renderCrossReference(){const x=data.cross_reference||{},s=x.summary||{},metrics=$("crossReferenceMetrics"),leads=$("crossReferenceLeads"),chains=$("crossReferenceChains");metrics.append(metric(fmt(s.entities),"typed entities","info"),metric(fmt(s.relationships),"cross-model relationships","good"),metric(fmt(s.component_relationship_fusions),"component edge fusions"),metric(fmt(s.semantic_profiles),"semantic profiles","info"),metric(fmt(s.compound_exposure_chains),"compound exposure chains",s.compound_exposure_chains?"warning":"good"),metric(fmt(s.multi_source_fusions),"multi-source edges",s.multi_source_fusions?"good":""),metric(fmt(s.runtime_observed_fusions),"runtime-observed edges",s.runtime_observed_fusions?"info":""),metric(fmt(s.finding_chains),"finding chains"),metric(fmt(s.review_leads),"review leads",s.review_leads?"warning":"good"));renderBars("crossReferenceConsensus",s.classifications||{});renderBars("crossReferenceSemanticCoverage",s.semantic_dimensions||{});renderBars("crossReferenceCompoundExposure",s.compound_exposures_by_kind||{});(x.review_leads||[]).slice(0,100).forEach(value=>{const entry=document.createElement("article");entry.className="citation-entry";entry.append(text("h3",value.kind.replaceAll("_"," ")),text("p",value.description||"Review required."));const meta=document.createElement("div");meta.className="citation-meta";meta.append(tag(value.priority||"unrated",value.priority==="high"?"error":"warning"));(value.subject_ids||[]).slice(0,4).forEach(subject=>meta.append(tag(subject,"info")));entry.append(meta);leads.append(entry)});if(!leads.children.length)leads.append(text("p","No cross-source discrepancies or compound semantic exposures were projected.","muted"));[...(x.finding_chains||[])].sort((a,b)=>a.linkage_completeness_percent-b.linkage_completeness_percent||a.finding_id.localeCompare(b.finding_id)).slice(0,100).forEach(value=>{const entry=document.createElement("article");entry.className="citation-entry";entry.append(text("h3",value.finding_id),text("p",`${value.component_id||"component unresolved"} · ${pct(value.linkage_completeness_percent)} linkage completeness`));const meta=document.createElement("div");meta.className="citation-meta";Object.entries(value.dimensions||{}).forEach(([label,present])=>meta.append(tag(`${label}: ${present?"linked":"open"}`,present?"accepted":"warning")));(value.compound_exposure_kinds||[]).forEach(label=>meta.append(tag(label.replaceAll("_"," "),"warning")));const action=document.createElement("button");action.className="btn no-print";action.dataset.openFinding=value.finding_id;action.textContent="Open finding";entry.append(meta,action);chains.append(entry)});if(!chains.children.length)chains.append(text("p","No finding chains were available.","muted"));$("crossReferenceLimits").append(...(x.limitations||[]).map(value=>text("li",value)));$("crossReferenceBinding").textContent=`Analysis ${x.analysis_state_sha256||"unbound"} · projection ${x.content_sha256||"unbound"} · complete JSON: ${x.report_projection?.complete_source||"not recorded"}`}
