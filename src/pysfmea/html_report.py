...
def _report_record(
    ...
    return {
        ...
        "evidence": _text_list(scanner.get("evidence", [])),
...
function searchable(r){return [r.id,r.component,r.path,r.failure_mode,r.trigger,r.local_effect,r.next_higher_effect,r.end_effect,r.rule_id,...r.requirements,...r.linked_hazards,...r.subsystems].join(" ").toLowerCase()}
...
initHeader();renderOverview();renderFindings();initTable();renderArchitecture();renderTraceability();renderSequences();initDiagrams();renderMethodology();initNavigation();
</script>
</body>
</html>
'''
