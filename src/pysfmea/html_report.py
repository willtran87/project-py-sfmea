    ...
    return {
        ...
        "repository_inventory": {
            **analysis.get("repository_inventory", {}),
            "entries": analysis.get("repository_inventory", {}).get("entries", [])[:5000],
            "entries_truncated_for_report": len(
                analysis.get("repository_inventory", {}).get("entries", [])
            )
            > 5000,
        },
...
.dialog-head>div:first-child{min-width:0;flex:1}.dialog-actions{display:flex;flex:0 0 auto;align-items:center;justify-content:flex-end;gap:7px;flex-wrap:wrap}.dialog-actions .btn{padding:6px 9px}.dialog-position{min-width:72px;text-align:center;color:var(--muted);font-size:11px;font-variant-numeric:tabular-nums}.copy-confirmed{color:var(--green);border-color:var(--green)}
...
function detailNavigationState(){const id=decodeURIComponent(location.hash.slice(1).split("/",2)[1]||""),preferred=filtered.length?filtered:data.records;let records=preferred,index=records.findIndex(value=>value.id===id);if(index<0){records=data.records;index=records.findIndex(value=>value.id===id)}return{records,index}}
...
async function copyDetailLink(){const button=$("detailCopy"),value=location.href;let copied=false;try{if(navigator.clipboard?.writeText){await navigator.clipboard.writeText(value);copied=true}}catch(error){copied=false}if(!copied)copied=fallbackCopy(value);button.textContent=copied?"Link copied":"Copy unavailable";button.classList.toggle("copy-confirmed",copied);setTimeout(()=>{button.textContent="Copy link";button.classList.remove("copy-confirmed")},1800)}
function initDetailNavigation(){const dialog=$("detailDialog");$("detailPrevious").addEventListener("click",()=>moveDetailRecord(-1));$("detailNext").addEventListener("click",()=>moveDetailRecord(1));$("detailCopy").addEventListener("click",copyDetailLink);dialog.addEventListener("cancel",()=>history.replaceState(null,"","#failure-modes"));new MutationObserver(()=>{if(dialog.open)refreshDetailNavigation()}).observe(dialog,{attributes:true,attributeFilter:["open"]});document.addEventListener("keydown",event=>{if(!dialog.open||!event.altKey)return;if(event.key==="ArrowLeft"){event.preventDefault();moveDetailRecord(-1)}else if(event.key==="ArrowRight"){event.preventDefault();moveDetailRecord(1)}});if(dialog.open)refreshDetailNavigation()}
