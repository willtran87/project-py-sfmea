...
function initCrossViewNavigation(){document.addEventListener("click",event=>{const button=event.target instanceof Element?event.target.closest("[data-open-diagram]"):null;if(!button)return;event.preventDefault();event.stopImmediatePropagation();const index=(data.diagrams||[]).findIndex(value=>value.id===button.dataset.openDiagram);if(index<0)return;setView("diagrams");$("diagramSelect").selectedIndex=index;renderGenericDiagram()},true)}
