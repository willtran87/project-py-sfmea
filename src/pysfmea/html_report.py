...
def _report_record(
    item: dict[str, Any], validation_rules: list[str]
) -> dict[str, Any]:
...
def build_html_report_data(
    ...
    *,
    ...
    return {
        ...
        "records": [
            _report_record(item, sorted(set(rules_by_item[str(item.get("id", ""))])))
...
@media print{body{background:#fff;color:#111;font-size:10pt}.shell{display:block}.sidebar,.topbar,.filter-panel,.table-foot,.no-print{display:none!important}.content{max-width:none;padding:0}.view[hidden]{display:block!important;break-before:page}.hero{color:#111;background:#fff;border:1px solid #bbb;box-shadow:none}.hero:after{display:none}.hero-meta span{border-color:#bbb;background:#fff}.card,.metric,.table-wrap{box-shadow:none;break-inside:avoid}.section-head{margin-top:24px}dialog{display:none}.table-wrap{overflow:visible}table{min-width:0;font-size:8pt}}
</style>
</head>
<body>
<div class="shell">
...
<main class="main">
  ...
  <div class="content">
    ...
    <section class="view" data-view="guidance" hidden><div class="section-head"><div><h2>Guidance-to-finding traceability</h2><p>Versioned authoritative sources and exact locators connected through curated rules to candidate findings.</p></div></div><div class="notice"><strong>Interpretation:</strong> <span id="guidanceNotice"></span></div><div class="metrics" id="guidanceMetrics"></div><div class="section-head" style="margin-top:28px"><div><h2>Source catalog</h2><p>Status and applicability are explicit. Legacy or domain-specific material is never presented as universal compliance.</p></div></div><div class="catalog-grid" id="guidanceSources"></div><div class="section-head" style="margin-top:28px"><div><h2>Used citation locators</h2><p>Each locator is curated and versioned; usage counts show how many active findings inherit the relationship.</p></div></div><div class="card"><div class="citation-list" id="guidanceCitations"></div></div></section>
...
initHeader();renderOverview();renderFindings();initTable();renderArchitecture();renderTraceability();renderSequences();initDiagrams();renderGuidance();renderMethodology();initNavigation();
</script>
</body>
</html>
'''
