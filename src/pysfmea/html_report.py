...
def _generated_test_governance_projection(
    obligations: list[dict[str, Any]],
    executions: list[dict[str, Any]],
    baseline_id: str,
) -> dict[str, Any]:
    """Expose LLM-origin execution/effectiveness state without inventing proposal evidence."""

    records: list[dict[str, Any]] = []
    for obligation in obligations:
        automation = obligation.get("automation", {})
        if automation.get("implementation_origin") != "llm_generated":
            continue
        exact_executions = [
            execution
            for execution in executions
            if execution.get("obligation_id") == obligation.get("id")
            and execution.get("baseline_id") == baseline_id
            and execution.get("test", {}).get("sha256") == automation.get("test_sha256")
        ]
        passing = [value for value in exact_executions if value.get("status") == "passed"]
        stimulated = [value for value in passing if value.get("stimulus_observed") is True]
        criteria_complete = [
            value
            for value in stimulated
        ...
        gates = {
            "exact_llm_registration": bool(
                automation.get("implementation_status") == "implemented"
                and automation.get("implemented_test_path")
                and automation.get("test_sha256")
            ),
            "restricted_execution_passed": bool(passing),
            "failure_stimulus_observed": bool(stimulated),
            "acceptance_criteria_passed": bool(criteria_complete),
            "independent_evidence_review": bool(independently_reviewed),
        }
        records.append(
            {
                "obligation_id": obligation.get("id", ""),
                "finding_id": obligation.get("finding_id", ""),
                "test_path": automation.get("implemented_test_path", ""),
                "test_sha256": automation.get("test_sha256", ""),
                "gates": gates,
                "passed_internal_gates": sum(gates.values()),
                "required_internal_gates": len(gates),
                "execution_ids": [value.get("id", "") for value in exact_executions],
                "proposal_and_publication_receipt": "external_artifacts_required",
                "status": "evidence_ready" if all(gates.values()) else "blocked",
            }
        )
    embedded, projection = _bounded_projection(records, 250)
    return {
        "summary": {
            "llm_generated_tests": len(records),
            "evidence_ready": sum(value["status"] == "evidence_ready" for value in records),
            "blocked": sum(value["status"] == "blocked" for value in records),
        },
        "records": embedded,
        "projection": projection,
        "notice": (
            "The report shows analysis-resident registration and evidence gates. Exact proposal "
            "and human publication receipts remain separately verified external artifacts."
        ),
    }
...
def _report_enhancement_projection(workbench: dict[str, Any]) -> dict[str, Any]:
    """Keep the workbench navigable without embedding every complete queue."""

    interfaces = workbench.get("interface_disposition_queue", {})
    surfaces = workbench.get("surface_models", {})
    surface_projection = {
        category: {
            "count": len(values) if isinstance(values, list) else 0,
            "records": values[:50] if isinstance(values, list) else [],
            "truncated": isinstance(values, list) and len(values) > 50,
        }
        for category, values in sorted(surfaces.items())
    }
    clusters = workbench.get("review_clusters", [])
    portfolio = workbench.get("evidence_portfolio", [])
    client_queue = interfaces.get("clients", [])
    server_queue = interfaces.get("servers", [])
    return {
        "format": workbench.get("format", ""),
        "content_sha256": workbench.get("content_sha256", ""),
        "summary": workbench.get("summary", {}),
        "capability_register": workbench.get("capability_register", []),
        "hardening_register": workbench.get("hardening_register", []),
        "post_hardening_register": workbench.get("post_hardening_register", []),
        "next_generation_register": workbench.get("next_generation_register", []),
        "product_outcome_register": workbench.get("product_outcome_register", []),
        "capability_attestations": workbench.get("capability_attestations", {}),
        "resolution_attestations": workbench.get("resolution_attestations", {}),
        "product_outcome_attestations": workbench.get(
            "product_outcome_attestations", {}
        ),
        "artifact_freshness": workbench.get("artifact_freshness", {}),
        "artifact_health": workbench.get("artifact_health", {}),
        "scope_patch": workbench.get("scope_patch", {}),
        "scope_preview": workbench.get("scope_preview", {}),
        "evidence_preflight": workbench.get("evidence_preflight", {}),
        "calibration_campaign": workbench.get("calibration_campaign", {}),
        "review_campaign": workbench.get("review_campaign", {}),
        "finding_consolidation_program": workbench.get(
            "finding_consolidation_program", {}
        ),
        "evidence_onboarding": workbench.get("evidence_onboarding", {}),
        "precision_program": workbench.get("precision_program", {}),
        "architecture_program": workbench.get("architecture_program", {}),
        "interface_program": workbench.get("interface_program", {}),
        "temporal_resilience_program": workbench.get("temporal_resilience_program", {}),
        "guidance_specificity_program": workbench.get(
            "guidance_specificity_program", {}
        ),
        "performance_ratchet": workbench.get("performance_ratchet", {}),
        "report_delivery_program": workbench.get("report_delivery_program", {}),
        "llm_governance_program": workbench.get("llm_governance_program", {}),
        "qualification_program": workbench.get("qualification_program", {}),
        "analysis_fidelity_program": workbench.get("analysis_fidelity_program", {}),
        "sequence_sfta_program": workbench.get("sequence_sfta_program", {}),
        "assurance_automation_program": workbench.get(
            "assurance_automation_program", {}
        ),
        "architecture_interface_program": workbench.get(
            "architecture_interface_program", {}
        ),
        "product_outcome_scorecard": workbench.get("product_outcome_scorecard", {}),
        "activation_progress": workbench.get("activation_progress", {}),
        "metric_provenance": workbench.get("metric_provenance", {}),
        "report_scale": workbench.get("report_scale", {}),
        "precision_risks": workbench.get("precision_risks", {}),
        "acceptance_targets": workbench.get("acceptance_targets", {}),
        "evidence_acquisition": workbench.get("evidence_acquisition", {}),
        "review_clusters": clusters[:100],
        "evidence_portfolio": portfolio[:100],
        "architecture_mapping_queue": {
            **workbench.get("architecture_mapping_queue", {}),
            "proposals": workbench.get("architecture_mapping_queue", {}).get(
                "proposals", []
            )[:100],
        },
        "interface_disposition_queue": {
            "clients": client_queue[:100],
            "servers": server_queue[:100],
            "authority": interfaces.get("authority", ""),
        },
        "surface_models": surface_projection,
        "guardrails": workbench.get("guardrails", []),
        "report_projection": {
            "scope": "bounded_interactive_view",
            "clusters": {"total": len(clusters), "embedded": min(len(clusters), 100)},
            "portfolio": {
                "total": len(portfolio),
                "embedded": min(len(portfolio), 100),
            },
            "client_dispositions": {
                "total": len(client_queue),
                "embedded": min(len(client_queue), 100),
            },
            "server_dispositions": {
                "total": len(server_queue),
                "embedded": min(len(server_queue), 100),
            },
            "hardening_items": {
                "total": len(workbench.get("hardening_register", [])),
                "embedded": len(workbench.get("hardening_register", [])),
            },
            "post_hardening_items": {
                "total": len(workbench.get("post_hardening_register", [])),
                "embedded": len(workbench.get("post_hardening_register", [])),
            },
            "next_generation_items": {
                "total": len(workbench.get("next_generation_register", [])),
                "embedded": len(workbench.get("next_generation_register", [])),
            },
            "product_outcome_items": {
                "total": len(workbench.get("product_outcome_register", [])),
                "embedded": len(workbench.get("product_outcome_register", [])),
            },
            "complete_source": "sfmea enhance ANALYSIS -o enhancement-workbench.json",
        },
    }
...
def build_html_report_data(
    ...
    *,
    ...
    if profile not in {"engineering", "compact", "management"}:
        raise ValueError("profile must be engineering, compact, or management")
    profile_limit = {"engineering": max_records, "compact": 500, "management": 250}[
        profile
    ]
    max_records = min(max_records, profile_limit)
    ...
    sfta_model = build_sfta(analysis)
    cross_reference = build_cross_reference_index(
        analysis,
        assurance_register=assurance,
        guidance_trace=guidance_trace,
        sfta_model=sfta_model,
        architecture=graph,
    ...
    sfta = _report_sfta_projection(sfta_model)
    authoring_history = (
        analysis.get("sfta_authoring", {}).get("history", [])
        if isinstance(analysis.get("sfta_authoring"), dict)
        else []
    )
    if not isinstance(authoring_history, list):
        authoring_history = []
    sfta["authoring"] = {
        "history": authoring_history[-25:],
        "total_applies": len(authoring_history),
        "truncated": len(authoring_history) > 25,
        "workflow": [
            "sfmea sfta-authoring-init ANALYSIS",
            "sfmea sfta-authoring-seal DRAFT --analysis ANALYSIS",
            "sfmea sfta-authoring-verify SEALED --analysis ANALYSIS",
            "sfmea sfta-authoring-apply ANALYSIS SEALED",
        ],
        "notice": (
            "Fault-tree replacements require a named reviewer, rationale, exact "
            "analysis binding, and an approved sealed input."
        ),
    }
    ...
    generated_test_governance = _generated_test_governance_projection(
        selected_obligations,
        assurance.get("executions", []),
    ...
    diagrams = [
        *build_diagram_models(
            ...
            cross_reference_index=cross_reference,
        ),
        *(custom_diagrams or []),
    ]
    ...
    diagnostic_model = analysis_diagnostics(analysis)
    enhancement_model = _report_enhancement_projection(
        enhancement_workbench(analysis, diagnostics=diagnostic_model)
    )
    activation_state = analysis.get("activation", {})
    if not isinstance(activation_state, dict):
        activation_state = {}
    activation_history = activation_state.get("decision_history", [])
    if not isinstance(activation_history, list):
        activation_history = []
    consolidation_state = analysis.get("finding_consolidation", {})
    ...
    return {
        ...
        "diagnostics": diagnostic_model,
        "enhancement_workbench": enhancement_model,
        "activation": {
            "last_workspace_sha256": activation_state.get("last_workspace_sha256", ""),
            "last_applied_at": activation_state.get("last_applied_at", ""),
            "decision_history": activation_history[-500:],
            "decision_history_total": len(activation_history),
            "authority": activation_state.get("authority", ""),
            "finding_consolidation": {
                "format": consolidation_state.get("format", ""),
                "summary": consolidation_state.get("summary", {}),
                "records": consolidation_records[-100:],
                "records_total": len(consolidation_records),
                "records_truncated": len(consolidation_records) > 100,
                "authority": consolidation_state.get("authority", ""),
            },
        },
        "distributions": {
        ...
        "assurance": {
            ...
            "generated_test_governance": generated_test_governance,
            "report_projection": {
                "scope": "bounded_interactive_view",
        ...
        "cross_reference": {
            "format": cross_reference["format"],
            "analysis_state_sha256": cross_reference["analysis_state_sha256"],
            "content_sha256": cross_reference["content_sha256"],
            "authority": cross_reference["authority"],
            "summary": cross_reference["summary"],
            "component_relationship_fusions": cross_reference[
                "component_relationship_fusions"
            ][:500],
            "semantic_profiles": cross_reference["semantic_profiles"][:500],
            "verification_readiness_profiles": cross_reference[
            ...
            "adapter_provenance": {
                ...
                "adapter_ledger_entity_id": cross_reference["adapter_provenance"][
                ...
                "adapter_run_profiles": [
                    {
                        ...
                        "status": value["status"],
                        "linked_contribution_count": len(
                            value["linked_contribution_entity_ids"]
                        ),
                        "unlinked_contribution_count": len(
                            value["unlinked_contribution_entity_ids"]
                        ),
                    }
                    for value in cross_reference["adapter_provenance"][
                        "adapter_run_profiles"
                    ]
                ],
            ...
            "machine_assistance_provenance": {
                ...
                "unresolved_evidence_references": cross_reference[
                    "machine_assistance_provenance"
                ]["unresolved_evidence_references"][:500],
                "unresolved_citation_references": cross_reference[
                    "machine_assistance_provenance"
                ]["unresolved_citation_references"][:500],
            ...
            "guidance_provenance": {
                ...
                "methodology_sha256": cross_reference["guidance_provenance"][
                    "methodology_sha256"
                ],
                "source_profiles": cross_reference["guidance_provenance"][
                    "source_profiles"
                ],
                "citation_profiles": cross_reference["guidance_provenance"][
                    "citation_profiles"
                ][:500],
                "review_check_profiles": cross_reference["guidance_provenance"][
                    "review_check_profiles"
                ],
                "unresolved_methodology_source_ids": cross_reference[
                    "guidance_provenance"
                ]["unresolved_methodology_source_ids"],
                "mismatched_methodology_source_ids": cross_reference[
                    "guidance_provenance"
                ]["mismatched_methodology_source_ids"],
                "unresolved_citation_source_ids": cross_reference[
                    "guidance_provenance"
                ]["unresolved_citation_source_ids"],
                "notice": cross_reference["guidance_provenance"]["notice"],
            },
            "system_context_provenance": {
                "system_context_entity_id": cross_reference[
                    "system_context_provenance"
                ...
                "unresolved_catalog_claim_entity_ids": cross_reference[
                    "system_context_provenance"
                ]["unresolved_catalog_claim_entity_ids"][:500],
                "uncataloged_claim_entity_ids": cross_reference[
                    "system_context_provenance"
                ]["uncataloged_claim_entity_ids"][:500],
            ...
            "lifecycle_provenance": {
                ...
                "notice": cross_reference["lifecycle_provenance"]["notice"],
            },
        ...
        "configuration_review": {
            "component_mappings": len(context.get("component_mappings", [])),
            "guidance_rule_mappings": len(context.get("guidance_rule_mappings", [])),
            "interface_dispositions": len(context.get("interface_dispositions", [])),
            "next_command": (
                "sfmea config-authoring-init ANALYSIS --config sfmea.toml"
            ),
            "authority": (
                "named_project_configuration_not_runtime_evidence_independent_approval_or_compliance"
            ),
        },
        ...
        "architecture": {
            ...
            "deployment_topology": {
                "summary": analysis.get("deployment_topology", {}).get("summary", {}),
                "nodes": analysis.get("deployment_topology", {}).get("nodes", [])[:100],
                "edges": analysis.get("deployment_topology", {}).get("edges", [])[:100],
                "limitations": analysis.get("deployment_topology", {}).get(
                    "limitations", []
                ),
            },
            "shared_fate": {
                "summary": analysis.get("shared_fate_analysis", {}).get("summary", {}),
                "regions": analysis.get("shared_fate_analysis", {}).get("regions", [])[
                    :100
                ],
                "limitations": analysis.get("shared_fate_analysis", {}).get(
                    "limitations", []
                ),
            },
            "hierarchy": {
                "summary": analysis.get("architecture_hierarchy", {}).get(
                    "summary", {}
                ),
                "nodes": analysis.get("architecture_hierarchy", {}).get("nodes", [])[
                    :100
                ],
                "limitations": analysis.get("architecture_hierarchy", {}).get(
                    "inheritance_rules", []
                ),
            },
            "graphify_reconciliation": {
                "summary": analysis.get("graphify_reconciliation", {}).get(
                    "summary", {}
                ),
                "source": analysis.get("graphify_reconciliation", {}).get("source", {}),
                "authority": analysis.get("graphify_reconciliation", {}).get(
                    "authority", ""
                ),
                "edges": analysis.get("graphify_reconciliation", {}).get("edges", [])[
                    :100
                ],
            },
            "report_projection": {
                "record_limit_per_collection": 100,
                "complete_source": "deployment_topology, shared_fate_analysis, and architecture_hierarchy in the JSON analysis",
            },
        },
...
@media(max-width:760px){.saved-view-panel{grid-template-columns:1fr 1fr}.saved-view-panel .field{grid-column:1/-1}}
@media(max-width:480px){.saved-view-panel{grid-template-columns:1fr}.saved-view-panel .field{grid-column:auto}}
@media(prefers-reduced-motion:reduce){html{scroll-behavior:auto}.sidebar,.diagram-node,.diagram-edge{transition:none!important}}
.skip-link{position:fixed;left:12px;top:8px;z-index:100;transform:translateY(-160%);padding:10px 14px;border-radius:8px;background:var(--brand2);color:#fff;font-weight:750}.skip-link:focus{transform:none}
...
.actions{align-items:center}.render-status{max-width:280px;overflow:hidden;text-overflow:ellipsis}@media(max-width:900px){.render-status{display:none}}
</style>
</head>
<body>
<a class="skip-link" href="#mainContent">Skip to report content</a>
<div class="shell">
<aside class="sidebar" aria-label="Report navigation">
  ...
  <nav class="nav" id="nav" aria-label="Report sections">
    ...
    <button data-view="cross-reference"><span class="icon">⌁</span>Evidence fabric</button>
    ...
    <button data-view="enhancements"><span class="icon">+</span>Enhancement workbench</button>
...
<main class="main" id="mainContent" tabindex="-1">
  <header class="topbar"><button class="btn mobile-menu" id="menuButton" aria-label="Open navigation">☰</button><div class="crumb"><strong id="topTitle"></strong><span id="topSubtitle"></span></div><div class="actions"><span class="tag info render-status" id="renderStatus" role="status" aria-live="polite">Preparing overview…</span><button class="btn" id="themeButton" aria-label="Toggle color theme">◐ <span class="label">Theme</span></button><button class="btn primary" id="printButton">Print / PDF</button></div></header>
  <div class="content">
    ...
    <section class="view" data-view="failure-modes" hidden><div class="section-head"><div><h2>Failure-mode explorer</h2><p>Search, filter, sort, and open any embedded candidate to inspect its evidence, effects, controls, actions, and review state.</p></div><div class="actions no-print"><details class="column-controls"><summary class="btn">Columns</summary><div class="column-menu" id="columnControls"></div></details><button class="btn" id="csvButton">Export filtered CSV</button></div></div><div class="filter-panel no-print"><div class="field search"><label for="search">Search</label><input id="search" type="search" maxlength="200" placeholder="ID, component, path, failure mode, effect…"></div><div class="field"><label for="priorityFilter">Priority</label><select id="priorityFilter"></select></div><div class="field"><label for="classFilter">Failure class</label><select id="classFilter"></select></div><div class="field"><label for="dispositionFilter">Disposition</label><select id="dispositionFilter"></select></div><div class="field"><label for="hazardFilter">Hazard</label><select id="hazardFilter"></select></div><div class="field"><label for="subsystemFilter">Subsystem</label><select id="subsystemFilter"></select></div><div class="field"><label for="sortFilter">Sort</label><select id="sortFilter"><option value="review">Review order</option><option value="priority">Priority</option><option value="rpn_desc">RPN, highest first</option><option value="severity_desc">Severity, highest first</option><option value="source">Source path</option><option value="component">Component</option><option value="disposition">Disposition</option></select></div><div class="field filter-action"><button class="btn" id="resetFilters" type="button">Reset view</button></div></div><div class="saved-view-panel no-print"><div class="field"><label for="savedViewSelect">Saved view</label><select id="savedViewSelect"><option value="">Choose a saved view</option></select></div><div class="field"><label for="savedViewName">View name</label><input id="savedViewName" maxlength="60" placeholder="e.g. High-priority interface review"></div><button class="btn" id="saveView" type="button">Save current</button><button class="btn" id="deleteView" type="button" disabled>Delete</button><button class="btn" id="shareView" type="button">Copy share link</button><span class="saved-view-status" id="savedViewStatus" role="status" aria-live="polite">Saved views stay in this browser; shared links contain only the current bounded filter state.</span></div><div class="table-wrap"><table id="failureTable"><caption class="sr-only">Failure modes matching the current filters</caption><thead><tr><th scope="col">Priority</th><th scope="col">ID / state</th><th scope="col">Component / source</th><th scope="col">Failure mode</th><th scope="col">Class</th><th scope="col">Review</th><th scope="col">S/O/D · RPN</th></tr></thead><tbody id="recordRows"></tbody></table><div class="empty" id="recordEmpty" hidden>No records match the current filters.</div></div><div class="table-foot"><span id="recordCount" aria-live="polite"></span><div class="pager" aria-label="Failure-mode result pages"><button class="btn" id="prevPage">Previous</button><button class="btn" id="nextPage">Next</button></div></div></section>
    <section class="view" data-view="assurance" hidden><div class="section-head"><div><h2>Executable assurance checklist</h2><p>Failure-mode verification obligations with explicit stimuli, oracles, acceptance criteria, sandbox commands, and evidence requirements.</p></div></div><div class="notice"><strong>Evidence boundary:</strong> <span id="assuranceNotice"></span></div><div class="metrics" id="assuranceMetrics"></div><div class="card"><div class="card-head"><div><h3>LLM-generated test governance</h3><p id="generatedTestNotice"></p></div></div><div class="metrics" id="generatedTestMetrics"></div><div class="citation-list" id="generatedTestRows"></div></div><div class="card" style="margin-top:16px"><div class="card-head"><div><h3>Verification obligation register</h3><p id="assuranceCount"></p></div></div><div class="citation-list" id="assuranceRows"></div></div><div class="card" style="margin-top:16px"><div class="card-head"><div><h3>As-run evidence</h3><p>Sandbox and imported executions remain unreviewed until every original acceptance criterion is adjudicated independently.</p></div></div><div class="citation-list" id="assuranceExecutions"></div></div></section>
    <section class="view" data-view="architecture" hidden><div class="section-head"><div><h2>Architecture and boundaries</h2><p>Configured interfaces, declared deployment topology, shared-fate candidates, and inherited subsystem traceability.</p></div></div><div class="metrics" id="architectureMetrics"></div><div class="grid three"><div class="card"><div class="card-head"><div><h3>Declared deployment</h3><p>Repository-backed entities and relationships; candidate placements require review.</p></div><button class="btn no-print" data-open-diagram="declared-deployment-topology">Open diagram</button></div><div class="citation-list" id="deploymentTopology"></div></div><div class="card"><div class="card-head"><div><h3>Shared-fate regions</h3><p>Conservative common-cause leads from shared deployment, subsystem, or dependency membership.</p></div><button class="btn no-print" data-open-diagram="shared-fate-regions">Open diagram</button></div><div class="citation-list" id="sharedFateRegions"></div></div><div class="card"><div class="card-head"><div><h3>Inherited hierarchy</h3><p>Nested subsystem and source-package trace aggregation without invented links.</p></div><button class="btn no-print" data-open-diagram="architecture-hierarchy">Open diagram</button></div><div class="citation-list" id="architectureHierarchy"></div></div></div><div class="card" style="margin-top:16px"><div class="card-head"><div><h3>System-interface flow</h3><p>Configured boundary statements. Arrow direction follows the supplied source and target.</p></div></div><div class="flow-list" id="interfaceFlows"></div></div><div class="section-head" style="margin-top:28px"><div><h2>Subsystem inventory</h2><p>Explicit subsystem mappings are used where present; otherwise the report groups components by source path.</p></div></div><div class="subsystem-grid" id="subsystemGrid"></div></section>
    <section class="view" data-view="cross-reference" hidden><div class="section-head"><div><h2>Cross-reference evidence fabric</h2><p>One deterministic chain across scanner relationships, runtime observations, semantic analyzers, guidance, SFTA, findings, quality diagnostics, verification obligations, test candidates, coverage, executions, and evidence.</p></div><button class="btn no-print" data-open-diagram="cross-reference-evidence-fabric">Open evidence diagram</button></div><div class="notice"><strong>Interpretation boundary:</strong> Populated links, model intersections, and multi-source agreement improve review leverage; quality diagnostics are workflow evidence, test references and coverage are not verification evidence, and no link establishes reachability, causality, completeness, compliance, verification success, or risk acceptance.</div><div class="metrics" id="crossReferenceMetrics"></div><div class="grid"><div class="card"><div class="card-head"><div><h3>Relationship consensus</h3><p>Independent channels fused by directed component pair.</p></div></div><div class="bars" id="crossReferenceConsensus"></div></div><div class="card"><div class="card-head"><div><h3>Semantic exposure coverage</h3><p>Components linked to exact bounded analyzer records.</p></div></div><div class="bars" id="crossReferenceSemanticCoverage"></div></div><div class="card"><div class="card-head"><div><h3>Compound model intersections</h3><p>Failure-mode chains where independently derived semantic models co-occur.</p></div></div><div class="bars" id="crossReferenceCompoundExposure"></div></div><div class="card"><div class="card-head"><div><h3>Review governance</h3><p>Finding-local diagnostics, change state, revalidation, and governed next action.</p></div></div><div class="bars" id="crossReferenceReviewGovernance"></div></div><div class="card"><div class="card-head"><div><h3>Quality diagnostics</h3><p>All workflow diagnostics by level; analysis-scope findings remain separately counted.</p></div></div><div class="bars" id="crossReferenceQualityDiagnostics"></div></div><div class="card"><div class="card-head"><div><h3>Source-change state</h3><p>Change classification retained alongside review and assurance state.</p></div></div><div class="bars" id="crossReferenceSourceChanges"></div></div><div class="card"><div class="card-head"><div><h3>Verification lifecycle</h3><p>Governed next-step state without implying approval or authorization to execute.</p></div></div><div class="bars" id="crossReferenceVerificationLifecycle"></div></div><div class="card"><div class="card-head"><div><h3>Verification evidence posture</h3><p>Candidate links, observations, implementations, executions, and reviewed evidence remain distinct.</p></div></div><div class="bars" id="crossReferenceEvidencePosture"></div></div><div class="card"><div class="card-head"><div><h3>Verification readiness gaps</h3><p>Accepted findings requiring ownership, implementation, execution, review, or revalidation work.</p></div></div><div class="bars" id="crossReferenceReadinessGaps"></div></div><div class="card"><div class="card-head"><div><h3>Prioritized discrepancies and intersections</h3><p>Scanner disagreement, quality blockers, semantic intersections, and assurance-readiness gaps that deserve review.</p></div></div><div class="citation-list" id="crossReferenceLeads"></div></div></div><div class="card" style="margin-top:16px"><div class="card-head"><div><h3>Finding-to-evidence chains</h3><p>Lowest linkage-completeness chains are shown first. Completeness is relationship accounting only.</p></div></div><div class="citation-list" id="crossReferenceChains"></div></div><div class="card" style="margin-top:16px"><div class="card-head"><h3>Authority and limits</h3></div><ul id="crossReferenceLimits"></ul><p class="small mono" id="crossReferenceBinding"></p></div></section>
    ...
    <section class="view" data-view="enhancements" hidden>
      <div class="section-head"><div><h2>Enhancement and evidence activation workbench</h2><p>Prioritized acquisition recipes, root-cause clusters, verification portfolios, static surface models, and governed disposition queues.</p></div></div>
      <div class="notice"><strong>Authority boundary:</strong> Recipes are inert until explicitly executed in an approved sandbox. Static candidates, representative clusters, mappings, evidence, and decisions retain their separate review authority.</div>
      <div class="metrics" id="enhancementMetrics"></div>
      <div class="grid"><div class="card"><div class="card-head"><div><h3>Artifact freshness</h3><p>Exact baseline and state bindings; freshness is not evidence sufficiency.</p></div></div><div class="citation-list" id="enhancementFreshness"></div></div><div class="card"><div class="card-head"><div><h3>Acceptance targets</h3><p>Proposed measurable product targets requiring representative evidence.</p></div></div><div class="citation-list" id="enhancementTargets"></div></div></div>
      <div class="grid"><div class="card"><div class="card-head"><div><h3>Evidence-scope patch</h3><p>Review-only configuration preview; the report never writes project configuration.</p></div></div><div class="citation-list" id="enhancementScopePatch"></div></div><div class="card"><div class="card-head"><div><h3>Calibration campaign</h3><p>Bounded sampling workload, not statistical validation.</p></div></div><div class="citation-list" id="enhancementCalibration"></div></div></div>
      <div class="grid"><div class="card"><div class="card-head"><div><h3>Evidence acquisition</h3><p>Repository-specific argv recipes; no repository code was executed to create them.</p></div></div><div class="citation-list" id="enhancementEvidence"></div></div><div class="card"><div class="card-head"><div><h3>Static surface coverage</h3><p>Review-lead counts, not runtime reachability or control effectiveness.</p></div></div><div class="bars" id="enhancementSurfaces"></div></div></div>
      <div class="grid"><div class="card"><div class="card-head"><div><h3>Highest-value review clusters</h3><p>Representatives accelerate triage but never apply a decision to every member.</p></div></div><div class="citation-list" id="enhancementClusters"></div></div><div class="card"><div class="card-head"><div><h3>Verification portfolio</h3><p>Shared high-priority obligation groups for efficient hardening.</p></div></div><div class="citation-list" id="enhancementPortfolio"></div></div></div>
      <div class="card" style="margin-top:16px"><div class="card-head"><div><h3>Precision risks</h3><p>High-volume rules and unresolved reconciliation areas requiring calibration evidence.</p></div></div><div class="citation-list" id="enhancementPrecision"></div></div>
      <div class="card" style="margin-top:16px"><div class="card-head"><div><h3>Real-repository hardening register</h3><p>All 76 audit items have a concrete product resolution, measurable acceptance criterion, and explicit authority boundary.</p></div></div><div class="citation-list" id="enhancementHardening"></div></div>
      <div class="card" style="margin-top:16px"><div class="card-head"><div><h3>Post-hardening resolution register</h3><p>All 82 follow-on findings are projected through a versioned product, evidence, or human-authority path.</p></div></div><div class="citation-list" id="enhancementPostHardening"></div></div>
      <div class="grid" style="margin-top:16px"><div class="card"><div class="card-head"><div><h3>Operational resolution programs</h3><p>Review, evidence, precision, architecture, interface, timing, guidance, performance, reporting, LLM, and qualification activation state.</p></div></div><div class="citation-list" id="enhancementOperations"></div></div><div class="card"><div class="card-head"><div><h3>Review campaign</h3><p>Deterministic root-cause representatives and stratified calibration samples. No disposition is applied automatically.</p></div></div><div class="citation-list" id="enhancementReviewCampaign"></div></div></div>
      <div class="grid" style="margin-top:16px"><div class="card"><div class="card-head"><div><h3>Closed-loop activation</h3><p>Integrity-bound evidence onboarding and named decisions applied to this analysis.</p></div></div><div class="citation-list" id="enhancementActivation"></div></div><div class="card"><div class="card-head"><div><h3>Product outcome register</h3><p>The complete E001-E095 outcome classification with evidence and authority boundaries.</p></div></div><div class="citation-list" id="enhancementOutcomes"></div></div></div>
      <div class="card" style="margin-top:16px"><div class="card-head"><div><h3>Real-run resolution register</h3><p>All 102 recommendations have an implemented product capability, executable project-evidence gate, or named human-authority gate.</p></div></div><div class="citation-list" id="enhancementResolution"></div></div>
      <div class="card" style="margin-top:16px"><div class="card-head"><div><h3>Complete capability register</h3><p>Product capabilities stay distinct from missing project evidence and named human authority.</p></div></div><div class="citation-list" id="enhancementCapabilities"></div></div>
    </section>
    ...
    <section class="view" data-view="sfta" hidden><div class="section-head"><div><h2>Hazard and Software Fault Tree explorer</h2><p>Explicit top-down logical models correlated with bottom-up SFMEA candidates. Placeholder trees expose missing decomposition without inventing causality.</p></div></div><div class="notice"><strong>Interpretation boundary:</strong> <span id="sftaNotice"></span></div><div class="metrics" id="sftaMetrics"></div><div class="grid"><div class="card"><div class="card-head"><div><h3>Fault trees</h3><p>Gate logic is shown only when supplied through governed configuration or the authoring workflow.</p></div></div><div class="citation-list" id="sftaTrees"></div></div><div class="card"><div class="card-head"><div><h3>Bidirectional reconciliation gaps</h3><p id="sftaGapCount">Top-down events without findings and hazard-linked findings without tree events.</p></div></div><div class="citation-list" id="sftaGaps"></div></div><div class="card"><div class="card-head"><div><h3>Governed authoring history</h3><p>Named approvals and exact-analysis-bound replacements applied through the closed loop.</p></div></div><div class="citation-list" id="sftaAuthoring"></div></div></div></section>
...
const filterDefaults={page:1,pageSize:100,search:"",priority:"",failureClass:"",disposition:"",hazard:"",subsystem:"",sort:"review"};
const filterState={...filterDefaults};let filtered=[];
const filterKeys=["search","priority","failureClass","disposition","hazard","subsystem","sort"];
const filterControls={search:"search",priority:"priorityFilter",failureClass:"classFilter",disposition:"dispositionFilter",hazard:"hazardFilter",subsystem:"subsystemFilter",sort:"sortFilter"};
const filterQueryKeys={search:"q",priority:"p",failureClass:"c",disposition:"d",hazard:"h",subsystem:"s",sort:"o"};
const savedViewStorageKey=`pysfmea.saved-views.${document.querySelector('meta[name="pysfmea-analysis-baseline"]')?.content||"unbound"}`;
let savedViews=[];
...
function savedViewStatus(message){$("savedViewStatus").textContent=message}
function boundedFilterState(candidate={}){const clean={...filterDefaults};filterKeys.forEach(key=>{const value=typeof candidate[key]==="string"?candidate[key].slice(0,key==="search"?200:120):"";if(key==="search")clean[key]=value;else{const control=$(filterControls[key]);clean[key]=[...control.options].some(option=>option.value===value)?value:filterDefaults[key]}});return clean}
function syncFilterControls(){filterKeys.forEach(key=>{$(filterControls[key]).value=filterState[key]})}
function applyFilterState(candidate){Object.assign(filterState,boundedFilterState(candidate),{page:1});syncFilterControls();applyFilters()}
function sharedFilterState(){return Object.fromEntries(filterKeys.filter(key=>filterState[key]!==filterDefaults[key]).map(key=>[filterQueryKeys[key],filterState[key]]))}
function sharedFilterHash(){const params=new URLSearchParams(sharedFilterState());return `#failure-modes${params.size?`?${params.toString()}`:""}`}
function filterStateFromHash(){const marker=location.hash.indexOf("?");if(marker<0)return null;const params=new URLSearchParams(location.hash.slice(marker+1)),candidate={};filterKeys.forEach(key=>{const value=params.get(filterQueryKeys[key]);if(value!==null)candidate[key]=value});return candidate}
function loadSavedViews(){try{const parsed=JSON.parse(localStorage.getItem(savedViewStorageKey)||"[]");savedViews=Array.isArray(parsed)?parsed.filter(value=>value&&typeof value.name==="string"&&value.state&&typeof value.state==="object").slice(0,20).map(value=>({name:value.name.slice(0,60),state:boundedFilterState(value.state)})):[]}catch{savedViews=[];savedViewStatus("Browser storage is unavailable; share links remain available.")}renderSavedViews()}
function persistSavedViews(){try{localStorage.setItem(savedViewStorageKey,JSON.stringify(savedViews.slice(0,20)));return true}catch{savedViewStatus("This browser did not allow the saved view to be stored.");return false}}
function renderSavedViews(selected=""){const select=$("savedViewSelect"),current=selected||select.value;select.replaceChildren(new Option("Choose a saved view",""));savedViews.forEach((value,index)=>select.append(new Option(value.name,String(index))));select.value=savedViews[Number(current)]?String(Number(current)):"";$("deleteView").disabled=!select.value}
function saveCurrentView(){const name=$("savedViewName").value.trim().replace(/\s+/g," ").slice(0,60);if(!name){savedViewStatus("Enter a name before saving this view.");$("savedViewName").focus();return}const existing=savedViews.findIndex(value=>value.name.toLowerCase()===name.toLowerCase()),entry={name,state:boundedFilterState(filterState)};if(existing>=0)savedViews[existing]=entry;else{savedViews.unshift(entry);if(savedViews.length>20)savedViews.length=20}if(persistSavedViews()){renderSavedViews(String(existing>=0?existing:0));savedViewStatus(`Saved “${name}” for this analysis baseline.`)}}
function deleteCurrentView(){const index=Number($("savedViewSelect").value);if(!Number.isInteger(index)||!savedViews[index])return;const name=savedViews[index].name;savedViews.splice(index,1);if(persistSavedViews()){renderSavedViews();$("savedViewName").value="";savedViewStatus(`Deleted “${name}”.`)}}
async function copyShareLink(){const url=`${location.href.split("#")[0]}${sharedFilterHash()}`;history.replaceState(null,"",sharedFilterHash());try{await navigator.clipboard.writeText(url);savedViewStatus("Share link copied. It contains filter values only; saved-view names stay local.")}catch{const helper=document.createElement("textarea");helper.value=url;helper.setAttribute("readonly","");helper.className="sr-only";document.body.append(helper);helper.select();const copied=document.execCommand("copy");helper.remove();savedViewStatus(copied?"Share link copied.":`Copy this share link: ${url}`)}}
...
function resetFailureModeView(){applyFilterState(filterDefaults);$("savedViewSelect").value="";$("deleteView").disabled=true;savedViewStatus("Filters reset to the default review order.");$("search").focus()}
function initTable(){initColumnControls();optionize("priorityFilter",data.records.map(r=>r.priority),"All priorities");optionize("classFilter",data.records.map(r=>r.failure_class),"All classes");optionize("dispositionFilter",data.records.map(r=>r.disposition),"All dispositions");optionize("hazardFilter",data.catalog.hazards.map(h=>h.id),"All hazards");optionize("subsystemFilter",data.records.flatMap(r=>r.subsystems),"All subsystems");[["priorityFilter","priority"],["classFilter","failureClass"],["dispositionFilter","disposition"],["hazardFilter","hazard"],["subsystemFilter","subsystem"],["sortFilter","sort"]].forEach(([id,key])=>$(id).addEventListener("change",e=>{filterState[key]=e.target.value;filterState.page=1;applyFilters()}));$("search").addEventListener("input",e=>{filterState.search=e.target.value.slice(0,200);filterState.page=1;applyFilters()});$("resetFilters").addEventListener("click",resetFailureModeView);$("saveView").addEventListener("click",saveCurrentView);$("deleteView").addEventListener("click",deleteCurrentView);$("shareView").addEventListener("click",copyShareLink);$("savedViewSelect").addEventListener("change",e=>{const entry=savedViews[Number(e.target.value)];$("deleteView").disabled=!entry;if(entry){$("savedViewName").value=entry.name;applyFilterState(entry.state);savedViewStatus(`Loaded “${entry.name}”.`)}});$("prevPage").addEventListener("click",()=>{if(filterState.page>1){filterState.page--;renderTable()}});$("nextPage").addEventListener("click",()=>{if(filterState.page*filterState.pageSize<filtered.length){filterState.page++;renderTable()}});loadSavedViews();const shared=filterStateFromHash();if(shared){applyFilterState(shared);savedViewStatus("Applied the bounded filter state from this share link.")}else applyFilters()}
...
function renderCrossReference(){
  ...
  metrics.append(
    ...
    metric(fmt(exceptionSummary.handlers_may_reraise_original),"handlers that may rethrow original",exceptionSummary.handlers_may_reraise_original?"warning":"good"),
    metric(fmt(exceptionDispositions.caught_with_conditional_reraise),"conditional rethrow edges",exceptionDispositions.caught_with_conditional_reraise?"warning":"good"),
    metric(fmt(exceptionDispositions.caught_with_conditional_translation),"conditional translation edges",exceptionDispositions.caught_with_conditional_translation?"warning":"good"),
    ...
    metric(fmt(s.machine_suggestions),"machine suggestions",s.proposed_machine_suggestions?"warning":"info"),
    metric(fmt(s.machine_summaries),"machine summaries",s.stale_machine_summaries?"warning":"info"),
    metric(fmt(s.machine_claim_relationships),"machine claim comparisons",s.machine_claim_relationships?"warning":"good"),
    metric(fmt(s.findings_with_machine_assistance),"findings with machine-assistance links","info"),
    metric(fmt((s.machine_assistance_unresolved_evidence_references||0)+(s.machine_assistance_unresolved_citation_references||0)),"unresolved machine references",(s.machine_assistance_unresolved_evidence_references||s.machine_assistance_unresolved_citation_references)?"danger":"good"),
    metric(fmt(s.guidance_sources),"versioned guidance sources","info"),
    metric(fmt(s.guidance_citations_with_source_lineage),"citations with source lineage",s.guidance_citations_with_source_lineage===s.guidance_citations?"good":"warning"),
    metric(fmt(s.findings_with_complete_guidance_lineage),"findings with complete guidance lineage",s.findings_with_complete_guidance_lineage===s.findings_with_guidance_citations?"good":"warning"),
    metric(fmt(s.unresolved_guidance_source_references),"unresolved guidance source references",s.unresolved_guidance_source_references?"danger":"good"),
    metric(fmt(s.finding_context_claims),"finding context claims","info"),
    metric(fmt(s.matched_finding_context_claims),"exact context matches",s.matched_finding_context_claims?"good":"info"),
    metric(fmt(s.unmatched_finding_context_claims),"context alignment review leads",s.unmatched_finding_context_claims?"warning":"good"),
    metric(fmt(s.findings_with_explicit_system_context),"findings with explicit context","info"),
    metric(fmt((s.analysis_lifecycle_events||0)+(s.finding_review_events||0)+(s.subject_lifecycle_events||0)),"digest-bound lifecycle events","info"),
    metric(fmt(s.unresolved_lifecycle_subject_references),"unresolved lifecycle subjects",s.unresolved_lifecycle_subject_references?"warning":"good"),
    ...
    metric(fmt(s.runtime_observed_fusions),"runtime-observed edges",s.runtime_observed_fusions?"info":""),
    metric(fmt(s.finding_chains),"finding chains"),
    metric(fmt(s.review_leads),"review leads",s.review_leads?"warning":"good")
  );
  renderBars("crossReferenceConsensus",s.classifications||{});
  renderBars("crossReferenceSemanticCoverage",s.semantic_dimensions||{});
  renderBars("crossReferenceCompoundExposure",s.compound_exposures_by_kind||{});
  renderBars("crossReferenceReviewGovernance",s.review_governance_states||{});
  renderBars("crossReferenceQualityDiagnostics",s.quality_diagnostics_by_level||{});
  renderBars("crossReferenceSourceChanges",s.source_change_states||{});
  ...
  (x.review_leads||[]).slice(0,100).forEach(value=>{
    ...
    const meta=document.createElement("div");meta.className="citation-meta";
    meta.append(tag(value.priority||"unrated",value.priority==="high"?"error":"warning"));
    (value.subject_ids||[]).slice(0,4).forEach(subject=>meta.append(tag(subject,"info")));
    entry.append(meta);leads.append(entry)
  });
  if(!leads.children.length)leads.append(text("p","No cross-source discrepancies, compound semantic exposures, or readiness gaps were projected.","muted"));
  [...(x.finding_chains||[])].sort((a,b)=>a.linkage_completeness_percent-b.linkage_completeness_percent||a.finding_id.localeCompare(b.finding_id)).slice(0,100).forEach(value=>{
    const entry=document.createElement("article");entry.className="citation-entry";
    entry.append(text("h3",value.finding_id),text("p",`${value.component_id||"component unresolved"} · ${pct(value.linkage_completeness_percent)} linkage completeness`));
    const meta=document.createElement("div");meta.className="citation-meta";
    Object.entries(value.dimensions||{}).forEach(([label,present])=>meta.append(tag(`${label}: ${present?"linked":"open"}`,present?"accepted":"warning")));
    ...
    if((value.machine_assistance_entity_ids||[]).length)meta.append(tag(`${value.machine_assistance_entity_ids.length} machine-assisted link${value.machine_assistance_entity_ids.length===1?"":"s"}`,"warning"));
    if(value.guidance_lineage_status&&value.guidance_lineage_status!=="not_applicable")meta.append(tag(`guidance lineage: ${value.guidance_lineage_status}`,value.guidance_lineage_status==="complete"?"accepted":"error"));
    (value.system_context_alignment_statuses||[]).forEach(status=>meta.append(tag(`context: ${status.replaceAll("_"," ")}`,status==="matched"?"accepted":"warning")));
    if((value.lifecycle_event_entity_ids||[]).length)meta.append(tag(`${value.lifecycle_event_entity_ids.length} review-history event${value.lifecycle_event_entity_ids.length===1?"":"s"}`,"info"));
    if(value.verification_lifecycle_state)meta.append(tag(`lifecycle: ${value.verification_lifecycle_state.replaceAll("_"," ")}`,"info"));
    if(value.verification_evidence_posture)meta.append(tag(`evidence: ${value.verification_evidence_posture.replaceAll("_"," ")}`,value.verification_evidence_posture.startsWith("verified_")?"accepted":"warning"));
    if(value.verification_next_action_id&&value.verification_next_action_id!=="none")meta.append(tag(`next: ${value.verification_next_action_id.replaceAll("_"," ")}`,"info"));
    (value.verification_readiness_gaps||[]).forEach(label=>meta.append(tag(label.replaceAll("_"," "),"error")));
    (value.compound_exposure_kinds||[]).forEach(label=>meta.append(tag(label.replaceAll("_"," "),"warning")));
    const action=document.createElement("button");action.className="btn no-print";action.dataset.openFinding=value.finding_id;action.textContent="Open finding";
...
function openFindingFromTrace(findingId){const record=data.records.find(value=>value.id===findingId);if(!record)return;setView("failure-modes");filterState.search=findingId;filterState.page=1;$("search").value=findingId;applyFilters();openRecord(record)}
...
function renderSfta(){const s=data.sfta||{},r=s.reconciliation||{},m=r.summary||{},metrics=$("sftaMetrics"),treeRoot=$("sftaTrees"),gapRoot=$("sftaGaps");$("sftaNotice").textContent=s.notice||"Fault-tree logic requires explicit engineering input and review.";metrics.append(metric(fmt(m.hazards),"hazards"),metric(fmt(m.explicit_trees),"explicit trees",m.explicit_trees?"good":""),metric(fmt(m.approved_trees),"exact approved trees",m.approved_trees?"good":"warning"),metric(fmt(m.qualitative_cut_sets),"qualitative cut sets",m.qualitative_cut_sets?"info":""),metric(fmt(m.placeholder_trees),"undeveloped trees",m.placeholder_trees?"danger":"good"),metric(fmt(m.findings_correlated_to_events),"correlated findings"),metric(fmt(m.top_down_uncovered_events),"uncovered events",m.top_down_uncovered_events?"danger":"good"),metric(fmt(m.bottom_up_unmapped_findings),"unmapped findings",m.bottom_up_unmapped_findings?"danger":"good"));(s.trees||[]).forEach(tree=>{const entry=document.createElement("article"),events=(tree.nodes||[]).filter(v=>v.kind==="event"),gates=(tree.nodes||[]).filter(v=>v.kind==="gate"),linked=new Set(events.flatMap(v=>v.linked_finding_ids||[])),cuts=tree.cut_set_analysis||{};entry.className="citation-entry";entry.append(text("h3",`${tree.hazard_id} · ${tree.top_event}`),text("p",tree.description||"No description supplied."));const meta=document.createElement("div");meta.className="citation-meta";meta.append(tag(tree.source==="explicit_configuration"?"explicit logic":"undeveloped placeholder",tree.source==="explicit_configuration"?"accepted":"warning"),tag(tree.logic_status||"review state unavailable",tree.logic_status==="approved_for_qualitative_cut_sets"?"accepted":"warning"),tag(`${events.length} events`,`info`),tag(`${gates.length} gates`,`info`),tag(`${linked.size} correlated findings`,`info`),tag(`${fmt(cuts.cut_set_count)} cut sets`,cuts.status==="computed"?"info":"warning"));entry.append(meta,text("p",cuts.notice||"Qualitative cut sets are unavailable.","small"));(cuts.cut_sets||[]).slice(0,25).forEach(value=>entry.append(text("p",`${value.id}: ${value.event_ids.join(" + ")}${value.contains_undeveloped?" · includes undeveloped event":""}${value.contains_external?" · includes external event":""}`,"small mono")));if((cuts.cut_set_count||0)>25)entry.append(text("p",`Showing 25 of ${fmt(cuts.cut_set_count)} cut sets; use the machine-readable SFTA output for the complete bounded result.`,"notice"));const index=(data.diagrams||[]).findIndex(v=>v.metadata?.category==="sfta"&&v.metadata?.tree_id===tree.id);if(index>=0){const show=document.createElement("button");show.className="btn";show.textContent="Open fault-tree diagram";show.addEventListener("click",()=>{$("diagramSelect").selectedIndex=index;renderGenericDiagram();setView("diagrams")});entry.append(show)}treeRoot.append(entry)});if(!treeRoot.children.length)treeRoot.append(text("p","No hazards are configured; no fault trees can be developed.","muted"));const groups=[["Top-down event has no bottom-up finding",r.top_down_uncovered_events||[],"warning"],["Hazard-linked finding has no tree event",r.bottom_up_unmapped_findings||[],"warning"],["Tree correlation conflicts with hazard link",r.hazard_link_mismatches||[],"error"]];groups.forEach(([label,values,kind])=>values.slice(0,250).forEach(value=>{const entry=document.createElement("article");entry.className="citation-entry";entry.append(text("h3",label),text("p",value.description||value.failure_mode||value.finding_id||value.event_id||"Gap requires review."));const meta=document.createElement("div");meta.className="citation-meta";meta.append(tag(kind,kind),tag(value.hazard_id||"hazard not recorded","info"));if(value.tree_id)meta.append(tag(value.tree_id,"info"));if(value.event_id)meta.append(tag(value.event_id,"info"));if(value.finding_id)meta.append(tag(value.finding_id,"info"));entry.append(meta);gapRoot.append(entry)}));if(!gapRoot.children.length)gapRoot.append(text("p","No bidirectional SFTA reconciliation gaps were identified for the configured model.","muted"))}
function renderSftaAuthoring(){const a=data.sfta?.authoring||{},root=$("sftaAuthoring");(a.history||[]).slice().reverse().forEach(value=>{const entry=document.createElement("article"),hazards=value.replacement_hazards||[];entry.className="citation-entry";entry.append(text("h3",`${hazards.length} hazard tree replacement(s)`),text("p",`Applied ${value.applied_at||"at an unrecorded time"}.`));const meta=document.createElement("div");meta.className="citation-meta";meta.append(tag("approved and applied","accepted"));(value.reviews||[]).forEach(review=>meta.append(tag(`${review.hazard_id}: ${review.reviewer||"reviewer not recorded"}`,"info")));entry.append(meta,text("p",hazards.join(", ")||"No replacement hazard IDs recorded.","small mono"));root.append(entry)});if(!root.children.length)root.append(text("p","No governed SFTA replacement has been applied to this analysis.","muted"),text("p",(a.workflow||[]).join(" → ")||"Use the SFTA authoring workflow to create a reviewable draft.","small mono"))}
...
function setView(view,updateHash=true){ensureViewRendered(view);document.querySelectorAll(".view").forEach(node=>node.hidden=node.dataset.view!==view);document.querySelectorAll("#nav button").forEach(button=>{const active=button.dataset.view===view;button.classList.toggle("active",active);if(active)button.setAttribute("aria-current","page");else button.removeAttribute("aria-current")});const button=document.querySelector(`#nav button[data-view="${view}"]`),icon=button?.querySelector(".icon")?.textContent||"";$("topSubtitle").textContent=button?button.textContent.replace(icon,"").trim():"Report";if(updateHash)history.replaceState(null,"",`#${view}`);document.body.classList.remove("menu-open");window.scrollTo({top:0,behavior:"smooth"})}
function decodeHashPart(value){try{return decodeURIComponent(value)}catch{return value}}
function initNavigation(){document.querySelectorAll("#nav button").forEach(button=>button.addEventListener("click",()=>setView(button.dataset.view)));const requestedHash=location.hash,route=location.hash.slice(1).split("?",1)[0],parts=route.split("/").map(decodeHashPart),initial=parts[0],reference=parts[1],nodeId=parts[2],hasInitial=Boolean(document.querySelector(`.view[data-view="${initial}"]`));setView(hasInitial?initial:"overview",false);if(hasInitial){if(initial==="failure-modes"&&reference){const record=data.records.find(value=>value.id===reference);if(record)openRecord(record,false)}else if(initial==="assurance"&&reference)openAssuranceForFinding(reference,false);else if(initial==="diagrams"&&reference){const index=(data.diagrams||[]).findIndex(value=>value.id===reference);if(index>=0){$("diagramSelect").selectedIndex=index;renderGenericDiagram();if(nodeId){const node=(activeDiagram.nodes||[]).find(value=>value.id===nodeId);if(node){showDiagramNode(node,false);const element=[...document.querySelectorAll("#diagramStage .diagram-node")].find(value=>value.dataset.id===node.id);markTraceTarget(element)}else if(nodeId.startsWith("failure:"))openPropagationForFinding(nodeId.slice("failure:".length),false)}}}if(requestedHash)history.replaceState(null,"",requestedHash)}$("menuButton").addEventListener("click",()=>document.body.classList.toggle("menu-open"));$("themeButton").addEventListener("click",()=>{const root=document.documentElement;root.dataset.theme=root.dataset.theme==="dark"?"light":"dark"});$("printButton").addEventListener("click",preparePrint);$("detailClose").addEventListener("click",()=>{$("detailDialog").close();history.replaceState(null,"","#failure-modes")});$("csvButton").addEventListener("click",exportCsv)}
...
function renderDiagnostics(){const d=data.diagnostics||{},q=d.qualification||{},metrics=$("diagnosticQualification"),actions=$("diagnosticActions"),calibration=$("reviewCalibration"),review=d.workload?.review_calibration||{};if(!metrics||!actions||!calibration)return;metrics.append(metric(`${q.overall_grade||"?"} · ${pct(q.overall_score)}`,"overall diagnostic readiness",q.overall_score>=90?"good":q.overall_score>=70?"warning":"danger"));(q.domains||[]).forEach(value=>metrics.append(metric(`${value.grade} · ${pct(value.score)}`,value.id.replaceAll("_"," "),value.status==="strong"?"good":value.status==="attention"?"warning":"danger")));(d.recommended_actions||[]).forEach(value=>{const entry=document.createElement("article");entry.className="citation-entry";entry.append(text("h3",`${value.priority} · ${value.id}`),text("p",value.reason),text("p",value.command,"small mono"));actions.append(entry)});if(!actions.children.length)actions.append(text("p","No diagnostic actions are currently projected.","muted"));calibration.append(text("p",`${fmt(review.reviewed)} reviewed · ${fmt(review.unreviewed)} unreviewed · ${pct(review.reviewed_percent)} complete`));Object.entries(review.dispositions||{}).sort((a,b)=>b[1]-a[1]).forEach(([label,value])=>calibration.append(text("p",`${label.replaceAll("_"," ")}: ${fmt(value)}`,"small")))}
function renderEnhancementWorkbench(){
  const w=data.enhancement_workbench||{},s=w.summary||{};
  const metrics=$("enhancementMetrics"),evidence=$("enhancementEvidence"),surfaces=$("enhancementSurfaces"),clusters=$("enhancementClusters"),portfolio=$("enhancementPortfolio"),capabilities=$("enhancementCapabilities"),hardening=$("enhancementHardening"),postHardening=$("enhancementPostHardening"),resolution=$("enhancementResolution"),operations=$("enhancementOperations"),reviewCampaign=$("enhancementReviewCampaign"),activation=$("enhancementActivation"),outcomes=$("enhancementOutcomes"),freshness=$("enhancementFreshness"),targets=$("enhancementTargets"),precision=$("enhancementPrecision"),scopePatch=$("enhancementScopePatch"),calibration=$("enhancementCalibration");
  if(!metrics||!evidence||!surfaces||!clusters||!portfolio||!capabilities||!hardening||!postHardening||!resolution||!operations||!reviewCampaign||!activation||!outcomes||!freshness||!targets||!precision||!scopePatch||!calibration)return;
  const maturity=s.product_outcome_maturity||{};
  metrics.append(metric(fmt(s.enhancements),"capabilities accounted","good"),metric(fmt(s.hardening_items),"hardening items accounted"),metric(fmt(s.post_hardening_items),"post-hardening items"),metric(fmt(s.next_generation_items),"real-run recommendations"),metric(fmt(s.product_outcome_items),"outcomes maturity-assessed","info"),metric(fmt(maturity.implemented),"implemented outcomes","good"),metric(fmt(maturity.partial),"partial outcomes",maturity.partial?"warning":"good"),metric(fmt(maturity.planned),"planned outcomes",maturity.planned?"danger":"good"),metric(fmt(maturity.validated),"independently validated",maturity.validated?"good":"warning"),metric(fmt(s.review_clusters),"review clusters"),metric(pct(s.cluster_reduction_percent),"clustering reduction",s.cluster_reduction_percent?"good":""),metric(fmt(s.portfolio_groups),"evidence portfolio groups"),metric(fmt(s.architecture_mapping_candidates),"mapping proposals",s.architecture_mapping_candidates?"warning":"good"),metric(fmt(w.product_outcome_attestations?.product_evidence_gaps),"maturity evidence gaps",w.product_outcome_attestations?.product_evidence_gaps?"danger":"good"),metric(fmt(w.product_outcome_attestations?.overclaim_gaps),"capability overclaims",w.product_outcome_attestations?.overclaim_gaps?"danger":"good"));
  const health=w.artifact_health||{},f=health.freshness||w.artifact_freshness||{};freshness.append(text("h3",`Overall · ${health.overall_status||f.status||"unknown"}`),text("p",`Baseline ${f.baseline_id||"missing"}`,"small mono"),text("p",`Analysis SHA-256 ${f.analysis_state_sha256||"missing"}`,"small mono"),text("p",`Completeness ${fmt(health.completeness?.passed)}/${fmt(health.completeness?.total)} · sufficiency ${pct(health.sufficiency?.percent)}`,"small"));[["run manifest",f.run_manifest],["coverage",f.coverage],["runtime imports",f.runtime_imports],["assurance executions",f.assurance_executions]].forEach(([label,value])=>{const row=document.createElement("div");row.className="citation-meta";row.append(tag(label,"info"),tag(value?.status||"missing",value?.status==="current"||value?.status==="current_scan_input"?"accepted":"warning"));freshness.append(row)});
  const patch=w.scope_patch||{};scopePatch.append(text("h3",patch.status||"not required"),text("pre",patch.toml_preview||"No evidence-scope patch is currently proposed.","small mono"),text("p",patch.authority,"small"));
  const campaign=w.calibration_campaign||{};calibration.append(text("h3",`${fmt(campaign.total_sample_remaining)} reviews remaining`),text("p",campaign.method,"small"));(campaign.campaigns||[]).slice(0,10).forEach(value=>calibration.append(text("p",`${value.rule_id}: ${fmt(value.remaining_sample)} of ${fmt(value.planning_sample_target)} planned samples remain`,"small")));
  const assigned=w.review_campaign||{};reviewCampaign.append(text("h3",`${fmt(assigned.units_projected)} projected review units`),text("p",`${fmt(assigned.cluster_units?.length)} cluster representatives · ${fmt(assigned.calibration_samples?.length)} calibration samples · ${fmt(assigned.batches?.length)} batches`,"small"),text("p",assigned.authority||"","small"));(assigned.batches||[]).slice(0,10).forEach(value=>reviewCampaign.append(text("p",`${value.id}: ${fmt(value.unit_count)} review units`,"small")));
  const applied=data.activation||{},history=applied.decision_history||[],consolidation=applied.finding_consolidation||{},consolidationSummary=consolidation.summary||{};activation.append(text("h3",`${fmt(applied.decision_history_total)} governed decisions retained`),text("p",`${fmt(consolidationSummary.canonical_groups)} canonical review groups · ${fmt(consolidationSummary.grouped_finding_memberships)} preserved finding memberships`,"small"),text("p",applied.last_applied_at?`Last applied ${applied.last_applied_at}`:"No activation workspace has been applied to this analysis.","small"),text("p",applied.last_workspace_sha256?`Workspace ${applied.last_workspace_sha256}`:"Start with: sfmea activate-init ANALYSIS REPOSITORY","small mono"));(consolidation.records||[]).slice(-10).reverse().forEach(value=>activation.append(text("p",`${value.id} / canonical ${value.canonical_finding_id} / ${fmt(value.member_count)} members`,"small mono")));history.slice(-20).reverse().forEach(value=>activation.append(text("p",`${value.kind} / ${value.subject_id} / ${value.decision} / ${value.reviewer}`,"small")));
  (w.product_outcome_register||[]).forEach(value=>{const entry=document.createElement("article");entry.className="citation-entry";entry.append(text("h3",`${value.id} / ${value.title}`),text("p",value.product_resolution||""),text("p",value.maturity_basis||"Maturity basis unavailable.","small"),text("p",`Limitation: ${(value.known_limitations||[]).join(" ")||"none recorded"}`,"small"),text("p",`Next: ${value.next_action||"complete the named acceptance criterion"}`,"small"),text("p",`Evidence: ${[...(value.implementation_evidence||[]),...(value.test_evidence||[])].join(", ")||"no implementation evidence claimed"}`,"small mono"));const meta=document.createElement("div");meta.className="citation-meta";const mature=value.product_maturity==="implemented"||value.product_maturity==="validated";meta.append(tag(value.product_maturity||"unassessed",mature?"accepted":value.product_maturity==="planned"?"danger":"warning"),tag(value.resolution_state,"warning"),tag(value.authority||"product","info"));entry.append(meta);outcomes.append(entry)});
  [["Closed-loop activation",w.activation_progress],["Finding consolidation",w.finding_consolidation_program],["Evidence onboarding",w.evidence_onboarding],["Evidence preflight",w.evidence_preflight],["Precision",w.precision_program],["Analysis fidelity",w.analysis_fidelity_program],["Sequences and SFTA",w.sequence_sfta_program],["Assurance automation",w.assurance_automation_program],["Architecture and interfaces",w.architecture_interface_program],["Timing and resilience",w.temporal_resilience_program],["Guidance",w.guidance_specificity_program],["Performance",w.performance_ratchet],["Report delivery",w.report_delivery_program],["LLM governance",w.llm_governance_program],["Qualification",w.qualification_program],["Outcome scorecard",w.product_outcome_scorecard]].forEach(([label,value])=>{const entry=document.createElement("article");entry.className="citation-entry";entry.append(text("h3",label),text("p",value?.format||"projection unavailable","small mono"),text("p",value?.authority||"","small"));if(label==="Assurance automation"&&value?.synthesized_test_designs){const synthesis=value.synthesized_test_designs,meta=document.createElement("div");meta.className="citation-meta";meta.append(tag(`property ${synthesis.property_tests?.status||"unavailable"}`,synthesis.property_tests?.status==="implemented"?"accepted":"warning"),tag(`contract ${synthesis.contract_tests?.status||"unavailable"}`,synthesis.contract_tests?.status==="implemented"?"accepted":"warning"),tag(synthesis.manifest_format||"format unavailable","info"));entry.append(meta,text("p",synthesis.authority||"","small"))}operations.append(entry)});
  (w.acceptance_targets?.targets||[]).forEach(value=>{const entry=document.createElement("article");entry.className="citation-entry";entry.append(text("h3",value.label),text("p",`${value.current??"unmeasured"} ${value.unit} · target ${value.comparison.replaceAll("_"," ")} ${value.target}`,"small"));const meta=document.createElement("div");meta.className="citation-meta";meta.append(tag(value.status,value.status==="met"?"accepted":"warning"));entry.append(meta);targets.append(entry)});
  (w.evidence_acquisition?.steps||[]).forEach(value=>{const entry=document.createElement("article");entry.className="citation-entry";entry.append(text("h3",`${value.priority} · ${value.id}`),text("p",value.reason),text("p",(value.argv||[]).join(" "),"small mono"),text("p",`Execution: ${value.execution_boundary}`,"small"));evidence.append(entry)});if(!evidence.children.length)evidence.append(text("p","No evidence acquisition steps are projected.","muted"));
  Object.entries(w.surface_models||{}).sort((a,b)=>b[1].count-a[1].count).forEach(([label,value])=>surfaces.append(text("p",`${label.replaceAll("_"," ")}: ${fmt(value.count)}${value.truncated?" · bounded":""}`,"small")));
  (w.review_clusters||[]).slice(0,25).forEach(value=>{const entry=document.createElement("article");entry.className="citation-entry";entry.append(text("h3",`${fmt(value.finding_count)} findings · ${value.rule_id}`),text("p",`${value.source_area} · ${value.shared_cause}`),text("p",`Representative ${value.representative_finding_id}`,"small mono"));clusters.append(entry)});if(!clusters.children.length)clusters.append(text("p","No active review clusters.","muted"));
  (w.evidence_portfolio||[]).slice(0,25).forEach(value=>{const entry=document.createElement("article");entry.className="citation-entry";entry.append(text("h3",`${value.verification_method} · ${fmt(value.finding_count)} findings`),text("p",`${value.rule_id} · ${value.source_area}`),text("p",`Priority score ${fmt(value.priority_score)} · ${value.hazard}`,"small"));portfolio.append(entry)});if(!portfolio.children.length)portfolio.append(text("p","No assurance obligations are available for portfolio planning.","muted"));
  (w.precision_risks?.insufficiently_calibrated_rules||[]).slice(0,25).forEach(value=>{const entry=document.createElement("article");entry.className="citation-entry";entry.append(text("h3",`${value.rule_id} · ${fmt(value.findings)} findings`),text("p",`${fmt(value.reviewed)} reviewed · acceptance ${pct(value.acceptance_percent)} · rejection ${pct(value.rejection_percent)}`,"small"));const meta=document.createElement("div");meta.className="citation-meta";meta.append(tag(value.status,"warning"));entry.append(meta);precision.append(entry)});if(!precision.children.length)precision.append(text("p","No insufficiently calibrated rule is projected.","muted"));
  (w.hardening_register||[]).forEach(value=>{const entry=document.createElement("article");entry.className="citation-entry";entry.append(text("h3",`${value.id} · ${value.title}`),text("p",value.product_resolution),text("p",`Acceptance: ${value.acceptance_criterion}`,"small"));const meta=document.createElement("div");meta.className="citation-meta";meta.append(tag(value.priority,"info"),tag(value.domain,"info"),tag(value.resolution_state,value.resolution_state==="resolved_product_capability"?"accepted":"warning"),tag(value.authority,"info"));entry.append(meta);hardening.append(entry)});
  (w.post_hardening_register||[]).forEach(value=>{const entry=document.createElement("article");entry.className="citation-entry";entry.append(text("h3",`${value.id} · ${value.title}`),text("p",value.product_resolution),text("p",`Acceptance: ${value.acceptance_criterion}`,"small"));const meta=document.createElement("div");meta.className="citation-meta";meta.append(tag(value.priority,"info"),tag(value.resolution_state,value.resolution_state==="resolved_product_projection"?"accepted":"warning"),tag(value.authority,"info"));entry.append(meta);postHardening.append(entry)});
  (w.next_generation_register||[]).forEach(value=>{const entry=document.createElement("article");entry.className="citation-entry";entry.append(text("h3",`${value.id} · ${value.title}`),text("p",value.product_resolution),text("p",`Acceptance: ${value.acceptance_criterion}`,"small"));const meta=document.createElement("div");meta.className="citation-meta";meta.append(tag(value.priority,"info"),tag(value.resolution_state,value.resolution_state==="resolved_product_capability"?"accepted":"warning"),tag(value.authority,"info"));entry.append(meta);resolution.append(entry)});
  (w.capability_register||[]).forEach(value=>{const entry=document.createElement("article");entry.className="citation-entry";entry.append(text("h3",`${value.id} · ${value.title}`),text("p",value.product_resolution));const meta=document.createElement("div");meta.className="citation-meta";meta.append(tag(value.domain,"info"),tag(value.status,value.status==="available_product_capability"?"accepted":"warning"),tag(value.authority,"info"));entry.append(meta);capabilities.append(entry)});
}
...
function renderConfigurationReview(view){const c=data.configuration_review||{},interfaceSummary=data.interface_reconciliation?.summary||{};if(view==="architecture")$("architectureMetrics").append(metric(fmt(c.component_mappings),"governed component mappings",c.component_mappings?"good":"warning"));else if(view==="guidance")$("guidanceMetrics").append(metric(fmt(c.guidance_rule_mappings),"project-reviewed rule mappings",c.guidance_rule_mappings?"good":""));else if(view==="coverage")$("contextMetrics").append(metric(fmt(interfaceSummary.applied_dispositions),"reviewed interface dispositions",interfaceSummary.unmatched_disposition_ids?"warning":interfaceSummary.applied_dispositions?"good":""))}
...
const viewOrder=[...document.querySelectorAll("#nav button[data-view]")].map(button=>button.dataset.view);
const viewRenderers=new Map([
  ["overview",()=>renderOverview()],
  ["coverage",()=>{renderCoverage();renderLanguageBoundaryMetrics();renderDiagnostics();renderInterfaceReconciliation();renderConfigurationReview("coverage")}],
  ["findings",()=>renderFindings()],
  ["failure-modes",()=>initTable()],
  ["assurance",()=>{renderAssuranceProgress();renderAssuranceWorkState()}],
  ["architecture",()=>{renderArchitecture();renderArchitectureModels();renderConfigurationReview("architecture")}],
  ["cross-reference",()=>renderCrossReference()],
  ["traceability",()=>renderTraceability()],
  ["sequences",()=>renderSequences()],
  ["diagrams",()=>initDiagrams()],
  ["guidance",()=>{renderGuidance();renderGuidanceGovernance();renderGuidanceApplicability();renderConfigurationReview("guidance")}],
  ["enhancements",()=>renderEnhancementWorkbench()],
  ["methodology",()=>renderMethodology()],
  ["sfta",()=>{renderSfta();renderSftaAuthoring();ensureViewRendered("diagrams")}],
  ["run-manifest",()=>{renderRunManifest();renderRunCacheMetrics()}],
]);
const renderedViews=new Set(),renderingViews=new Set(),viewRenderSeconds={},reportInitialView=viewOrder.includes(location.hash.slice(1).split("/",1)[0])?location.hash.slice(1).split("/",1)[0]:"overview";
let initialRenderSeconds=null;
function updateRenderingTelemetry(){const samples=viewOrder.filter(view=>Object.hasOwn(viewRenderSeconds,view)).map(view=>({view,state:renderedViews.has(view)?"ready":"error",render_seconds:viewRenderSeconds[view]}));window.__PYSFMEA_RENDERING__={mode:"progressive_on_demand",initial_view:reportInitialView,initial_ready:document.documentElement.dataset.reportReady==="true",rendered_view_count:renderedViews.size,total_view_count:viewOrder.length,all_views_ready:renderedViews.size===viewOrder.length,initial_render_seconds:initialRenderSeconds,maximum_view_render_seconds:samples.length?Math.max(...samples.map(value=>value.render_seconds)):null,samples,limitations:"Render timing uses browser performance.now() and excludes JSON parsing, browser startup, paint completion, native DOM memory, and GPU work."};const status=$("renderStatus");if(status)status.textContent=renderedViews.size===viewOrder.length?`All ${viewOrder.length} sections ready`:`On demand · ${renderedViews.size}/${viewOrder.length} sections ready`}
function ensureViewRendered(view){if(renderedViews.has(view))return true;if(renderingViews.has(view))return false;const renderer=viewRenderers.get(view),section=document.querySelector(`.view[data-view="${view}"]`);if(!renderer||!section)throw new Error(`No report renderer is registered for ${view}`);renderingViews.add(view);section.setAttribute("aria-busy","true");section.dataset.renderState="rendering";$("renderStatus").textContent=`Preparing ${view.replaceAll("-"," ")}…`;const started=performance.now();try{renderer();renderedViews.add(view);section.dataset.renderState="ready";return true}catch(error){section.dataset.renderState="error";const message=text("div",`This section could not be rendered: ${error instanceof Error?error.message:String(error)}`,"notice");message.setAttribute("role","alert");section.prepend(message);console.error(`PySFMEA report renderer failed for ${view}`,error);throw error}finally{const elapsed=Math.round((performance.now()-started)*1000)/1000000;viewRenderSeconds[view]=elapsed;if(initialRenderSeconds===null)initialRenderSeconds=elapsed;renderingViews.delete(view);section.removeAttribute("aria-busy");updateRenderingTelemetry()}}
function renderAllViews(){viewOrder.forEach(view=>ensureViewRendered(view));updateRenderingTelemetry()}
function preparePrint(){renderAllViews();window.print()}
function initCrossViewNavigation(){document.addEventListener("click",event=>{const target=event.target instanceof Element?event.target:null,diagramButton=target?.closest("[data-open-diagram]"),findingButton=target?.closest("[data-open-finding]");if(!diagramButton&&!findingButton)return;event.preventDefault();event.stopImmediatePropagation();if(findingButton){openFindingFromTrace(findingButton.dataset.openFinding);return}const index=(data.diagrams||[]).findIndex(value=>value.id===diagramButton.dataset.openDiagram);if(index<0)return;setView("diagrams");$("diagramSelect").selectedIndex=index;renderGenericDiagram()},true)}
const reportBootStarted=performance.now();
initHeader();renderProjectionNotices();initCrossViewNavigation();initNavigation();initDetailNavigation();
document.documentElement.dataset.reportReady="true";
document.documentElement.dataset.reportRenderMode="progressive-on-demand";
window.__PYSFMEA_REPORT_BOOT_SECONDS__=Math.round((performance.now()-reportBootStarted)*1000)/1000000;
updateRenderingTelemetry();
window.addEventListener("beforeprint",renderAllViews);
function renderAssuranceWorkState(){const values=(data.assurance?.obligations||[]).filter(value=>value.source_status==="active"),cards=[...document.querySelectorAll("#assuranceRows .citation-entry")];cards.forEach((card,index)=>{const work=values[index]?.work||{},meta=card.querySelector(".citation-meta");if(!work.state||!meta)return;meta.prepend(tag(`next: ${work.next_action_id||"none"}`,"info"));meta.prepend(tag(`work: ${work.state}`,work.automation_eligible?"accepted":"warning"));if((work.blockers||[]).length)card.append(text("h4","Work blockers"),list(work.blockers))})}
</script>
</body>
</html>
"""
