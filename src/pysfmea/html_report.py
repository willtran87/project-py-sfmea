...
function renderLanguageBoundaryMetrics(){const summary=data.repository_inventory?.summary||{},boundaries=summary.language_boundaries||{},metrics=$("contextMetrics");if(!metrics)return;metrics.append(metric(fmt(boundaries.files),"language-boundary files",boundaries.files?"info":""),metric(fmt(boundaries.imports),"boundary imports"),metric(fmt(boundaries.exports),"boundary exports"),metric(fmt(boundaries.literal_endpoints),"literal endpoints",boundaries.literal_endpoints?"warning":""))}
...
initHeader();renderOverview();renderCoverage();renderLanguageBoundaryMetrics();renderFindings();initTable();renderAssuranceProgress();renderSfta();renderProjectionNotices();renderRunManifest();renderRunCacheMetrics();renderArchitecture();renderTraceability();renderSequences();initDiagrams();renderGuidance();renderGuidanceApplicability();renderMethodology();initNavigation();initDetailNavigation();
