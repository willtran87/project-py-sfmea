...
| `static_control_flow_model` | `pysfmea-static-control-flow-model-1` | Count-reconciled, component-linked safe decisions for literal/boolean/`TYPE_CHECKING` branches, bounded literal sequence/mapping structural-pattern cases and guards, constant loops, empty literal iteration, guaranteed-nonempty literal iteration with terminal first-iteration bodies, exhaustive `try`/terminal-`finally` paths, terminal blocks, and short-circuit operands pruned before downstream call, exception, sequence, and failure-mode composition |
