...
| `static_control_flow_model` | `pysfmea-static-control-flow-model-2` | Count-reconciled, component-linked safe decisions for literal/boolean/`TYPE_CHECKING` branches; bounded exact-built-in numeric/sequence, indexing/slicing, boolean-value, conditional-value, deterministic collection-unpacking, mapping-union, and set-algebra expressions; literal sequence/mapping structural-pattern cases and guards; constant loops; empty literal iteration; guaranteed-nonempty literal iteration with terminal first-iteration bodies; exhaustive `try`/terminal-`finally` paths; terminal blocks; and short-circuit operands pruned before downstream composition. Format 2 makes the evaluator limits mandatory and tamper-checked |
