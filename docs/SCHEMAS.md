...
| `static_control_flow_model` | `pysfmea-static-control-flow-model-1` | Count-reconciled, component-linked safe decisions for literal/boolean/`TYPE_CHECKING` branches, empty literal iteration, terminal blocks, and short-circuit operands pruned before downstream call, exception, sequence, and failure-mode composition |
