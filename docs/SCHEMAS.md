...
| `assurance-test-generation-quality-corpus-v3` | Stratified artifact-backed campaign with pre-outcome selection, repository/framework/domain populations, concentration controls, and seeded-fault categories |
| `assurance-test-generation-fault-evidence` | Content-sealed baseline/seeded execution identities whose exact manifests and raw artifacts are reconciled during format-2/3 evaluation |
...
| `assurance-test-generation-quality-result-v3` | Content-sealed artifact replay with segment metrics, category diversity, 25 campaign gates, and exact format-3 corpus binding |
...
| `exception_propagation` | `pysfmea-exception-propagation-3` | Ordered, inheritance-aware raise/handler/finalizer records and resolved-call propagation edges with bounded branch outcomes, outcome certainty, active-binding rethrow identity, uniform-finalizer terminal basis, suppression/replacement disposition, uncertainty, and component/finding exposure. Format 3 makes complete bounded finalizer outcomes and their terminal-basis semantics part of the validated contract; only uniform safe terminal outcomes override propagation |
| `static_control_flow_model` | `pysfmea-static-control-flow-model-3` | Count-reconciled, component-linked safe decisions for literal/boolean/`TYPE_CHECKING` branches; bounded exact-built-in numeric/sequence, indexing/slicing, boolean-value, conditional-value, deterministic collection-unpacking, mapping-union, set-algebra, ASCII f-string formatting, and non-mutating allowlisted ASCII-string/bytes-method expressions; literal sequence/mapping structural-pattern cases and guards; constant loops; empty literal iteration; guaranteed-nonempty literal iteration with terminal first-iteration bodies; exhaustive `try`/terminal-`finally` paths; terminal blocks; and short-circuit operands pruned before downstream composition. Format 3 adds the mandatory, tamper-checked format-specification ceiling to the existing evaluator limits |
