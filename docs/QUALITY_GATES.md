...
|---|---|---|
| Focused mutation quality | Any aggregate population/score/debt regression, any invalid/skipped result, or any independently partitioned function group crossing its own population, score, or survivor limit | The current survivors remain explicit test-oracle debt and the selected functions are not whole-product mutation coverage. Per-function groups prevent a strong function from masking regression in a weak one. |
